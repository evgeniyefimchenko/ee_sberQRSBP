{if $order_info.payment_method.payment_id == $addons.ee_sberQRSBP.payment_id}	
	...
	{else}
		{$status = ''}
	{/if}
	{if $status && $status != 'В ПРОЦЕССЕ ОПЛАТЫ'}
		<h5 id="order-status">Статус QRCODE: {$status}</h5>
	{/if}
	{if !$decode || in_array($status, ['ОТОЗВАН', 'ВОЗВРАЩЕН', 'АННУЛИРОВАН', 'ОТКЛОНЕН', 'ИСТЕК'])}
        {* Генерация QR-кода *}
    ...
    {else}
		...
		{if $status == 'СОЗДАН'}
			{if !$decode.order_id}
				<span style="color: red;">Отсутствует order_id возврат невозможен!</span>
			{else}
				<a href="//{$config.https_host}/{$config.admin_index}/?dispatch=ee_sberQRSBP.revocation&order_id={$decode.order_id}"
				class="btn btn-primary" onclick="return confirm('Вы уверены, что хотите оформить возврат?')">Оформить возврат</a>
			{/if}
		{/if}
		<script>
